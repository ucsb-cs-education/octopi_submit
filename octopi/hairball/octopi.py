...
from kurt.scratch14 import Scratch14Plugin
...
from kurt.scratch14.user_objects import user_objects_by_name, UserObjectDef, make_user_objects
...
block_list = (list(make_blocks(kurt.scratch14.blockspecs_src.squeak_blockspecs)) +
...
scriptable_vars = user_objects_by_name['ScriptableScratchMorph'].defaults.copy()
scriptable_vars['hiddenbin'] = []
scriptable_vars['palettedict'] = {}
scriptable_vars['hidden'] = False
user_obj['ScriptableScratchMorph'] = UserObjectDef(1, 'BaseMorph', scriptable_vars)
user_obj['ScratchStageMorph'] = UserObjectDef(7, 'ScriptableScratchMorph', user_objects_by_name['ScratchStageMorph'].defaults.copy())
user_obj['ScratchSpriteMorph'] = UserObjectDef(5, 'ScriptableScratchMorph', user_objects_by_name['ScratchSpriteMorph'].defaults.copy())
user_obj['BlockMorph'] = UserObjectDef(3, 'BaseMorph', [('isSpecialForm', None), ('oldColor', None), ('hidden', False), ('inPalette', True)])
...
#user_obj['ReporterBlockMorph'] =  UserObjectDef(2, 'BaseMorph', [('isBoolean', False)])
...
class OctopiSerializer(Serializer):
    ...
    def load_scriptable(self, kurt_scriptable, voct_scriptable):
        ...
        # hidden scripts
        kurt_scriptable.hiddenscripts = map(self.load_script, voct_scriptable.hiddenbin)
        # fix comments
        comments = []
        blocks_by_id = []
        ...
        for script in kurt_scriptable.hiddenscripts:
            if isinstance(script, kurt.Comment):
                ...
                for block in reversed(list(kurt.scratch14.get_blocks_by_id(script))):
    ...
    def save_scriptable(self, kurt_scriptable, voct_scriptable):
        ...
        v14_scriptable.hiddenbin = map(self.save_script, kurt_scriptable.hiddenscripts)
        blocks_by_id = []
        for script in kurt_scriptable.hiddenscripts:
        ...
        def grab_comments(block):
            if block.comment:
                (x, y) = v14_scriptable.hiddenscripts[-1][0]
                pos = (x, y + 29)
